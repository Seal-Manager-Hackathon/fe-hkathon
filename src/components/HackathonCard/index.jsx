...
export default function HackathonCard({ hackathon }) {
  ...
  return (
    <Link
      ...
      className={cn(
        'group flex flex-row overflow-hidden rounded-xl bg-white',
        'border border-[#d8e0e6] transition-all duration-200',
        'hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(6,79,93,0.10)] hover:border-[#064f5d]/20'
      )}
    >
      {/* Thumbnail */}
      <div
      ...
      >
        ...
        <span className="relative z-10 select-none text-[24px] font-bold leading-none tracking-wide text-white/90 drop-shadow-sm">
          {initials}
        </span>
      </div>
      ...
      <div className="flex min-w-0 flex-1 items-center px-4 py-3 sm:px-5 sm:py-4">
        <div className="min-w-0 flex-1">
          ...
          {/* Title */}
          <h3 className={cn(
            'truncate text-[15px] font-bold text-[#1f2f3a] leading-snug',
            'group-hover:text-[#064f5d] transition-colors'
          )}>
            {hackathon.name}
          </h3>

          {/* Metadata */}
          {(hackathon.startTime || hackathon.endTime || hackathon.numberRound != null) && (
            <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-0.5">
              {(hackathon.startTime || hackathon.endTime) && (
                <span className="inline-flex items-center gap-1 text-[12px] text-[#6a7e8b]">
