...
export default function RegisterTeamsTab({ eventId }) {
  ...
  const columns = [
    ...
    { key: 'trackName', header: 'Track', headerIcon: FileText, render: (row) => row.trackId ? <Link to={`/admin/tracks/${row.trackId}`} className="text-[13px] font-medium text-[#064f5d] hover:underline">{row.trackName || '—'}</Link> : <span className="text-[13px] text-gray-400">—</span> },
    { key: 'topicTitle', header: 'Topic', headerIcon: FileText, render: (row) => row.topicId ? <Link to={`/admin/tracks/${row.trackId}/topics`} className="text-[13px] font-medium text-[#064f5d] hover:underline">{row.topicTitle || row.topicName || '—'}</Link> : <span className="text-[13px] text-gray-400">—</span> },
