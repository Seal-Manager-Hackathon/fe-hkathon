...
import { Search, Ban, Users, Eye, FileText, Trophy, Calendar, MoreHorizontal, CircleCheck } from 'lucide-react'
...
function regColumns() {
  return [
    ...
    { key: 'actions', header: 'Actions', headerIcon: MoreHorizontal, headerClassName: 'text-right', className: 'text-right', render: (row) => (<div className="flex items-center justify-end gap-2"><Link to={`/admin/register-teams/${row.id}`} className={viewBtnClass}><Eye className="h-3.5 w-3.5" /> View</Link></div>) },
  ]
}
  ...
  return (
    <>
      ...
      <div className="rounded-xl border border-[#e8ecf0] bg-white overflow-hidden">
        ...
        <BaseTable borderless columns={regColumns()} data={teams} page={pageIndex} pageSize={PAGE_SIZE} total={totalCount} onPageChange={setPageIndex} loading={loading} serverSide emptyText={hasActive ? 'No results match the current filters.' : 'No registered teams for this event.'} keyExtractor={(row) => row.id} minWidth="750px" />
      </div>
    </>
  )
}
