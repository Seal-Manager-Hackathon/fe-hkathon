...
export default function RegisterTeamDetail() {
  ...
  return (
    <div className="px-4 py-6 md:px-6 lg:px-8 lg:py-8">
      <div className="mb-4">
        <Link to={data.eventId ? `/admin/hackathons/${data.eventId}?tab=Register Teams` : '/admin/hackathons'} className="inline-flex cursor-pointer items-center gap-1.5 text-[13px] font-medium text-[#064f5d] hover:underline"><ArrowLeft className="h-4 w-4" /> Back to Hackathons</Link>
      </div>
      ...
      <div className="mb-6 rounded-2xl border border-[#e8ecf0] bg-white overflow-hidden shadow-sm">
        ...
        <div className="flex flex-wrap gap-4 border-t border-[#e8ecf0] bg-[#fafbfc] px-6 py-3 sm:px-8">
          ...
          <QuickStat icon={FileText} label="Track" value={data.trackTitle || '—'} href={data.trackId ? `/admin/tracks/${data.trackId}` : null} />
          <QuickStat icon={FileText} label="Topic" value={data.topicTitle || '—'} href={data.topicId && data.trackId ? `/admin/tracks/${data.trackId}/topics` : null} />
          ...
          <CardPanel title="Event & Track">
            <div className="divide-y divide-[#f5f5f5]">
              ...
              <InfoRow label="Track" icon={FileText}>
                {data.trackId ? <Link to={`/admin/tracks/${data.trackId}`} className="text-[14px] font-semibold text-[#064f5d] hover:underline">{data.trackTitle}</Link> : <span className="text-[14px] text-[#1f2f3a]">{data.trackTitle || '—'}</span>}
              </InfoRow>
              <InfoRow label="Topic" icon={FileText}>
                {data.topicId && data.trackId ? <Link to={`/admin/tracks/${data.trackId}/topics`} className="text-[14px] font-semibold text-[#064f5d] hover:underline">{data.topicTitle}</Link> : <span className="text-[14px] text-[#1f2f3a]">{data.topicTitle || '—'}</span>}
              </InfoRow>
