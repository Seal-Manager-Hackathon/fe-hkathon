...
const emptyItem = () => ({ name: '', description: '', score: 0 })
            ...
            {items.map((item, i) => (
              <div key={i} className="rounded-xl border border-[#e8ecf0] bg-white p-4">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <label className="mb-1 flex items-center gap-1.5 text-[12px] font-semibold text-gray-500"><Hash className="h-3 w-3" />Score</label>
                    <input type="number" value={item.score} onChange={(e) => updateItem(i, 'score', e.target.value)} min={0} className="field-input" />
                  </div>
                </div>
              </div>
            ))}
