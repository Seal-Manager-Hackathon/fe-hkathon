...
import { getEventLeaderboard } from '../../../../api/admin'
...
export default function EventLeaderboardTab({ eventId }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row) => (
        row.registerTeamId ? (
          <Link to={`/admin/register-teams/${row.registerTeamId}`} className="text-[14px] font-semibold text-[#064f5d] hover:underline truncate block">{row.teamName}</Link>
        ) : (
