...
export default function AwardDetail() {
  ...
  if (error || !award) return <div className="flex min-h-[60vh] flex-col items-center justify-center"><p className="text-[18px] font-semibold text-gray-500">{error || 'Award not found.'}</p><Link to={`/admin/hackathons/${eventId}?tab=Awards`} className="mt-4 text-[14px] font-medium text-[#064f5d] hover:underline">&larr; Back to Event</Link></div>
  ...
  return (
    <div className="px-4 py-6 md:px-6 lg:px-8 lg:py-8">
      <div className="mb-6">
        <Link to={`/admin/hackathons/${eventId}?tab=Awards`} className="inline-flex items-center gap-1.5 text-[14px] font-medium text-[#064f5d] hover:underline">&larr; Back to Event</Link>
      </div>
        ...
        <div>
          ...
          {event && (
            <p className="mt-1 text-[14px] text-gray-400">
              Event: <Link to={`/admin/hackathons/${eventId}`} className="font-medium text-[#064f5d] hover:underline">{event.name}</Link>
              <Badge label={event.status} className={`ml-2 ${statusBadge[event.status] || 'bg-[#f5f5f5] text-[#757575]'}`} />
            </p>
          )}
        </div>
        {!award.isDisable && (
          <Link to={`/admin/hackathons/${eventId}/awards/${awardId}/edit`} className="inline-flex items-center gap-2 rounded-lg bg-[#064f5d] px-4 py-2 text-[13px] font-semibold text-white hover:bg-[#05404a] sm:px-5 sm:py-2.5 sm:text-[14px] shrink-0 self-start sm:self-auto">
