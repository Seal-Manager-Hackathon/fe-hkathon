...
import { getChapterLeaderboard } from '../../../api/admin'
              ...
              {eventScores.map((es, i) => (
                <div key={es.eventId || i} className="flex items-center justify-between rounded-xl border border-[#e8ecf0] bg-[#fafbfc] px-4 py-3">
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <Link to={`/admin/hackathons/${es.eventId}`} className="text-[14px] font-semibold text-[#064f5d] hover:underline truncate">{es.eventName}</Link>
                  </div>
...
export default function ChapterLeaderboardPage() {
  ...
  const fetch = useCallback(async (page = 1) => {
    ...
    try {
      const result = await getChapterLeaderboard(year, { PageIndex: page, PageSize: PAGE_SIZE })
  ...
  const columns = [
    ...
    { key: 'teamName', header: 'Team', headerIcon: Users, headerClassName: 'w-[200px]', className: 'w-[200px]',
      render: (row) => (
        row.teamId ? (
          <Link to={`/admin/teams/${row.teamId}`} className="text-[14px] font-semibold text-[#064f5d] hover:underline truncate block">{row.teamName}</Link>
        ) : (
    ...
    { key: 'chapterScore', header: 'Chapter Score', headerIcon: Trophy, headerClassName: 'text-right w-[140px]', className: 'text-right w-[140px]',
      render: (row) => (<span className={cn('text-[16px] font-bold', row.rank === 1 ? 'text-[#b45309]' : row.rank === 2 ? 'text-[#475569]' : row.rank === 3 ? 'text-[#c2410c]' : 'text-[#064f5d]')}>{row.chapterScore != null ? row.chapterScore.toFixed(2) : '—'}</span>) },
