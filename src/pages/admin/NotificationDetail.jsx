    ...
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center">
        <p className="text-[18px] font-semibold text-gray-500">
          {isNotFound ? 'Không tìm thấy thông báo' : error || 'Notification not found.'}
        </p>
      ...
      <CardPanel title="Details">
        <div className="divide-y divide-[#f5f5f5]">
          ...
          <InfoRow label="Created At" icon={Calendar}>
            <p className="text-[14px] text-[#1f2f3a]">{formatDate(notification.createdAt)}</p>
          </InfoRow>
          {notification.updatedAt && (
            <InfoRow label="Updated At" icon={Clock}>
              <p className="text-[14px] text-[#1f2f3a]">{formatDate(notification.updatedAt)}</p>
            </InfoRow>
          )}
        </div>
      </CardPanel>
