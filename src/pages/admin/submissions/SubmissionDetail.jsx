          ...
          <SidebarCard icon={Layers} title="Context">
            ...
            <InfoRow label="Track" icon={FolderKanban}>
              {data.trackId && eventId ? (
                <Link to={`/admin/tracks/${data.trackId}`} className="text-[14px] font-semibold text-[#064f5d] hover:underline">{data.trackTitle || '—'}</Link>
              ) : <span className="text-[14px] text-gray-400">—</span>}
            </InfoRow>
            <InfoRow label="Topic" icon={FileText}>
              {data.topicId && data.trackId && eventId ? (
                <Link to={`/admin/tracks/${data.trackId}/topics`} className="text-[14px] font-semibold text-[#064f5d] hover:underline">{data.topicTitle || '—'}</Link>
              ) : <span className="text-[14px] text-gray-400">—</span>}
            </InfoRow>
          </SidebarCard>
        </div>
