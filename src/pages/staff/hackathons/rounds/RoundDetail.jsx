...
import { getRoundDetail, getEventDetail } from '../../../../api/staff'
...
export default function RoundDetail() {
  ...
  if (error || !round) return <div className="flex min-h-[60vh] flex-col items-center justify-center"><p className="text-[18px] font-semibold text-gray-500">{error || 'Round not found.'}</p><Link to="/staff/hackathons" className="mt-4 text-[14px] font-medium text-[#064f5d] hover:underline">&larr; Back to Hackathons</Link></div>

  return (
    <div className="px-4 py-6 md:px-6 lg:px-8 lg:py-8">
      <div className="mb-4">
        <Link to={event ? `/staff/hackathons/${event.id}?tab=Rounds` : '/staff/hackathons'} className="inline-flex cursor-pointer items-center gap-1.5 text-[13px] font-medium text-[#064f5d] hover:underline">&larr; Back to Event</Link>
      </div>
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          {event ? (
            <p className="mt-1 text-[14px] text-gray-400">
              Event: <Link to={`/staff/hackathons/${event.id}`} className="font-medium text-[#064f5d] hover:underline">{event.name}</Link>
        ...
        <Link to={`/staff/rounds/${roundId}/edit`} className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-[#064f5d] px-4 py-2 text-[13px] font-semibold text-white transition-colors hover:bg-[#05404a] sm:px-5 sm:py-2.5 sm:text-[14px] shrink-0 self-start sm:self-auto">
