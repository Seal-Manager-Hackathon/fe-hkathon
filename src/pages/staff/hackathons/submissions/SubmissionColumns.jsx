  ...
  return useMemo(
    () => [
      ...
      },
      {
        ...
        render: (row) =>
          row.roundId ? (
            <Link to={`/staff/hackathons/${eventId}?tab=Rounds`} className="text-[13px] font-medium text-[#064f5d] hover:underline">
              {row.roundName || '—'}
            </Link>
          ) : (
      ...
      },
      {
        ...
        render: (row) => {
          ...
          return (
            <Link to={`/staff/users/${s.userId}`} className="flex items-center gap-3 hover:opacity-80">
        ...
        render: (row) => {
          ...
          return (
            <div className="flex items-center justify-end gap-2">
              <Link to={`/staff/submissions/${row.lastSubmission.id}`} className={S.viewBtn}>
