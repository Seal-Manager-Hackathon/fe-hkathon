...
export default function TopicDetail() {
  ...
  const backUrl = track?.eventId
    ? `/staff/hackathons/${track.eventId}?tab=Tracks`
    : trackId ? `/staff/tracks/${trackId}` : '/staff/hackathons'
    ...
    <div className="px-4 py-6 md:px-6 lg:px-8 lg:py-8">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          {track && <p className="mt-1 text-[14px] text-gray-400">Track: <Link to={track?.eventId ? `/staff/hackathons/${track.eventId}?tab=Tracks` : '#'} className="font-medium text-[#064f5d] hover:underline">{track.title}</Link></p>}
        </div>
      </div>
