...
export default function TopicDetail() {
  ...
  if (error || !topic) return <div className="flex min-h-[60vh] flex-col items-center justify-center"><p className="text-[18px] font-semibold text-gray-500">{error || 'Topic not found.'}</p><Link to="/staff/hackathons" className="mt-4 text-[14px] font-medium text-[#064f5d] hover:underline">&larr; Back to Hackathons</Link></div>

  const backUrl = eventId && trackId
    ? `/staff/hackathons/${eventId}/tracks/${trackId}/topics`
    : '/staff/hackathons'
    ...
    <div className="px-4 py-6 md:px-6 lg:px-8 lg:py-8">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          {track && <p className="mt-1 text-[14px] text-gray-400">Track: <Link to={`/staff/hackathons/${eventId}/tracks/${trackId}`} className="font-medium text-[#064f5d] hover:underline">{track.title}</Link></p>}
        </div>
      </div>
    ...
    <div className="rounded-xl border border-[#e8ecf0] bg-white shadow-sm self-start overflow-hidden">
      ...
      <div className="divide-y divide-[#f5f5f5]">
        <div className="px-5 py-3.5">
          <Link to={`/staff/hackathons/${eventId}/tracks/${trackId}`} className="text-[14px] font-bold text-[#064f5d] hover:underline">{track.title}</Link>
        </div>
