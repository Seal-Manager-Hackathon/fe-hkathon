...
import { getTopicDetail, getTrackDetail, getEventDetail, updateTopic } from '../../../../api/staff'
...
export default function TopicEdit() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      navigate(`/staff/hackathons/${eventId}/tracks/${trackId}/topics`)
    } catch (err) {
  ...
  return (
    <EntityFormPage
      backUrl={`/staff/hackathons/${eventId}/tracks/${trackId}/topics`}
    ...
    <div className="space-y-4 self-start">
        ...
        <div className="rounded-xl border border-[#e8ecf0] bg-white shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-[#f5f5f5]">
            <div className="px-5 py-3.5">
              <Link to={`/staff/hackathons/${event.id}`} className="text-[14px] font-bold text-[#064f5d] leading-snug hover:underline">{event.name}</Link>
      ...
      {track && (
        <div className="rounded-xl border border-[#e8ecf0] bg-white shadow-sm overflow-hidden">
          ...
          <div className="space-y-2 px-5 py-3">
            <Link to={`/staff/hackathons/${eventId}/tracks/${trackId}`} className="text-[14px] font-bold text-[#064f5d] leading-snug hover:underline">{track.title}</Link>
