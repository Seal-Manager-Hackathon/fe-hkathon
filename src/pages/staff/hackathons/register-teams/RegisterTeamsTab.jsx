...
export default function RegisterTeamsTab({ eventId }) {
  ...
  const columns = [
    ...
    { key: 'roundName', header: 'Round', headerIcon: Layers, render: (row) => row.roundId ? <Link to={`/staff/rounds/${row.roundId}`} className="text-[13px] font-medium text-[#064f5d] hover:underline">{row.roundName || '—'}</Link> : <span className="text-[13px] text-gray-400">—</span> },
