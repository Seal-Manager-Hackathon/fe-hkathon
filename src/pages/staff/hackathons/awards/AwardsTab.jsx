...
import { getAwards, deleteAward, restoreAward, swapAward } from '../../../../api/staff'
  ...
  return [
    ...
    { key: 'actions', header: 'Actions', headerIcon: MoreHorizontal, headerClassName: 'text-right', className: 'text-right', render: (row) => (
      <div className="flex items-center justify-end gap-2">
        ...
        <Link to={`/staff/hackathons/${eventId}/awards/${row.id}`} className="inline-flex cursor-pointer items-center gap-1 rounded-lg bg-[#f5f5f5] px-2.5 py-1.5 text-[13px] font-semibold text-[#424242] hover:bg-[#e8e8e8]"><Eye className="h-3.5 w-3.5" />View</Link>
        {!row.isDisable && <Link to={`/staff/hackathons/${eventId}/awards/${row.id}/edit`} className={actionBtnClass}><Edit className="h-3.5 w-3.5" /> Edit</Link>}
  ...
  return (
    <>
      <div className="mb-4 flex items-center justify-between">
        ...
        <Link to={`/staff/hackathons/${eventId}/awards/create`} className="inline-flex cursor-pointer items-center gap-1.5 rounded-lg bg-[#064f5d] px-3 py-1.5 text-[12px] font-semibold text-white transition-colors hover:bg-[#05404a]"><Plus className="h-3.5 w-3.5" />Create Award</Link>
      </div>
