...
import { getEventLeaderboard } from '../../../../api/staff'
              ...
              {roundScores.map((rs, i) => (
                <div key={rs.roundNo || i} className="flex items-center justify-between rounded-xl border border-[#e8ecf0] bg-[#fafbfc] px-4 py-3">
                  <div className="flex items-center gap-2.5">
                    <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#e3f2fd] text-[12px] font-bold text-[#1565c0]">{rs.roundNo}</span>
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-[13px]">
            <div className="flex items-center gap-1.5">
              ...
              {team.trackId && eventId ? (
                <Link to={`/staff/hackathons/${eventId}/tracks/${team.trackId}`} className="font-semibold text-[#064f5d] hover:underline">{team.trackTitle || '—'}</Link>
              ) : (
            ...
            {team.topicTitle && (
              <div className="flex items-center gap-1.5">
                ...
                {team.topicId && team.trackId && eventId ? (
                  <Link to={`/staff/hackathons/${eventId}/tracks/${team.trackId}/topics`} className="font-semibold text-[#064f5d] hover:underline">{team.topicTitle}</Link>
                ) : (
...
export default function EventLeaderboardTab({ eventId }) {
  ...
  const fetch = useCallback(async (page = 1) => {
    ...
    setError('')
    try {
      const result = await getEventLeaderboard(eventId, { PageIndex: page, PageSize: PAGE_SIZE })
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row) => (
        row.registerTeamId ? (
          <Link to={`/staff/register-teams/${row.registerTeamId}`} className="text-[14px] font-semibold text-[#064f5d] hover:underline truncate block">{row.teamName}</Link>
        ) : (
    ...
    },
    {
      ...
      render: (row) => (
        row.trackId ? (
          <Link to={`/staff/hackathons/${eventId}/tracks/${row.trackId}`} className="text-[13px] font-semibold text-[#064f5d] hover:underline truncate block">{row.trackTitle || '—'}</Link>
        ) : (
    ...
    },
    {
      ...
      render: (row) => (
        row.topicTitle && row.topicId && row.trackId ? (
          <Link to={`/staff/hackathons/${eventId}/tracks/${row.trackId}/topics`} className="text-[13px] font-semibold text-[#064f5d] hover:underline truncate block">{row.topicTitle}</Link>
        ) : (
  ...
  return (
    <div>
      ...
      <div className="rounded-xl border border-[#e8ecf0] bg-white overflow-hidden">
        ...
        <BaseTable borderless columns={columns} data={items}
          page={meta.pageIndex} pageSize={PAGE_SIZE} total={meta.totalCount}
          onPageChange={fetch} loading={loading} serverSide
      ...
      <DetailModal open={!!detailTarget} team={detailTarget} eventId={eventId} onClose={() => setDetailTarget(null)} />
    </div>
  )
}
