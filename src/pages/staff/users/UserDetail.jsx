...
const eventColumns = [
  ...
  { key: 'trackTitle', header: 'Track', headerIcon: FileText, render: (row) => (
    row.trackId ? <Link to={`/staff/hackathons/${row.eventId}/tracks/${row.trackId}`} className="text-[13px] font-medium text-[#064f5d] hover:underline">{row.trackTitle || '—'}</Link> : <span className="text-[13px] text-gray-400">—</span>
  )},
  { key: 'topicTitle', header: 'Topic', headerIcon: FileText, render: (row) => (
    row.topicId && row.trackId ? <Link to={`/staff/hackathons/${row.eventId}/tracks/${row.trackId}/topics`} className="text-[13px] font-medium text-[#1f2f3a] hover:underline">{row.topicTitle || '—'}</Link> : <span className="text-[13px] text-gray-400">—</span>
  )},
...
export default function UserDetail() {
  ...
  if (error) {
    ...
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center">
        ...
        <Link to="/staff/users" className="mt-4 text-[14px] font-medium text-[#064f5d] hover:underline">&larr; Back to Users</Link>
      </div>
    )
  }
  ...
  return (
    <div className="px-4 py-6 md:px-6 lg:px-8 lg:py-8">
      <div className="mb-6">
        <Link to="/staff/users" className="inline-flex cursor-pointer items-center gap-1.5 text-[14px] font-medium text-[#064f5d] hover:underline">&larr; Back to Users</Link>
      </div>
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <Link to={`/staff/users/${id}/edit`} className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-[#064f5d] px-4 py-2.5 text-[14px] font-semibold text-white shadow-sm hover:bg-[#05404a] shrink-0 self-start"><Edit className="h-4 w-4" />Edit User</Link>
      </div>
