...
function registerColumns() {
  return [
    ...
    { key: 'trackName', header: 'Track', headerIcon: FileText, render: (row) => row.trackId ? <Link to={`/staff/hackathons/${row.eventId}/tracks/${row.trackId}`} className="text-[13px] font-medium text-[#064f5d] hover:underline">{row.trackName || '—'}</Link> : <span className="text-[13px] text-gray-400">—</span> },
    { key: 'topicName', header: 'Topic', headerIcon: FileText, render: (row) => row.topicId ? <Link to={`/staff/hackathons/${row.eventId}/tracks/${row.trackId}/topics/${row.topicId}`} className="text-[13px] font-medium text-[#064f5d] hover:underline">{row.topicName || '—'}</Link> : <span className="text-[13px] text-gray-400">—</span> },
...
export default function TeamDetail() {
  ...
  if (error) {
    ...
    return <div className="flex min-h-[60vh] flex-col items-center justify-center"><p className="text-[18px] font-semibold text-[#1f2f3a]">{nf ? 'Team not found' : error}</p><Link to="/staff/teams" className="mt-4 text-[14px] font-medium text-[#064f5d] hover:underline">&larr; Back to Teams</Link></div>
  }
  ...
  return (
    <div className="px-4 py-6 md:px-6 lg:px-8 lg:py-8">
      <div className="mb-4"><Link to="/staff/teams" className="inline-flex cursor-pointer items-center gap-1.5 text-[13px] font-medium text-[#064f5d] hover:underline"><ArrowLeft className="h-4 w-4" /> Back to Teams</Link></div>
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <Link to={`/staff/teams/${id}/edit`} className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-[#064f5d] px-4 py-2 text-[13px] font-semibold text-white hover:bg-[#05404a] sm:px-5 sm:py-2.5 sm:text-[14px] shrink-0 self-start sm:self-auto"><Edit className="h-4 w-4" />Edit Team</Link>
      </div>
        ...
        {subTab === 'submissions' && (
          <CardPanel title="Submissions">
            <div className="mb-4 px-1">
              ...
              {selectedReg && (
                <Link to={`/staff/register-teams/${selectedReg.id}`} className="ml-3 text-[13px] font-medium text-[#064f5d] hover:underline">View Registration</Link>
              )}
            </div>
