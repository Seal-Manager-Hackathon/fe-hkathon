...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      navigate(userData.role === 'Admin' ? '/admin' : userData.role === 'Staff' ? '/staff' : userData.role === 'Lecturer' ? '/lecture' : '/')
    } catch (err) {
      ...
      <div className="w-full max-w-[400px] rounded-2xl bg-white p-8 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="-mt-1 flex justify-end">
            <Link
              to="/forgot-password"
              className="text-[13px] font-medium text-[#064f5d] hover:underline"
            >
              Forgot password?
            </Link>
          </div>
