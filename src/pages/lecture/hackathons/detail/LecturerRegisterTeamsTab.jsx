...
import { Search, Ban, CircleCheck, Layers, Users, FileText, Calendar } from 'lucide-react'
...
export default function LecturerRegisterTeamsTab({ eventId }) {
  ...
  const columns = [
    { key: 'teamName', header: 'Team', headerIcon: Users, render: (row) => <span className="text-[14px] font-semibold text-[#1f2f3a]">{row.teamName || '—'}</span> },
    ...
    { key: 'roundName', header: 'Round', headerIcon: Layers, render: (row) => <span className="text-[13px] text-[#1f2f3a]">{row.roundName || '—'}</span> },
