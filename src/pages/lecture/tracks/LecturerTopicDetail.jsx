...
import { getLecturerTopicDetail, getLecturerTrackDetail } from '../../../api/lecturer'
...
export default function LecturerTopicDetail() {
  ...
  useEffect(() => {
    ...
    async function fetch() {
      ...
      try {
        const data = await getLecturerTopicDetail(topicId)
        if (cancelled) return
        setTopic(data)
        if (trackId) {
          try {
            const tr = await getLecturerTrackDetail(trackId)
  ...
  if (error || !topic) return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center">
      <p className="text-[18px] font-semibold text-gray-500">{error || 'Topic not found.'}</p>
  ...
  const backUrl = track?.eventId
    ...
    : trackId ? `/lecture/tracks/${trackId}` : '/lecture/hackathons'
        ...
        <div>
          ...
          {track && (
            <p className="mt-1 text-[14px] text-gray-400">
              Track: <Link to={track?.eventId ? `/lecture/hackathons/${track.eventId}?tab=Tracks` : '#'} className="font-medium text-[#064f5d] hover:underline">{track.title}</Link>
            </p>
          )}
        </div>
      </div>
          ...
          <CardPanel title="Topic Information">
            <div className="divide-y divide-[#f5f5f5]">
              ...
              <InfoRow label="Created At" icon={Calendar}>
                <p className="text-[14px] text-[#1f2f3a]">{formatDateTime(topic.createdAt)}</p>
              </InfoRow>
              <InfoRow label="Last Updated" icon={Clock}>
                <p className="text-[14px] text-[#1f2f3a]">{topic.updatedAt ? formatDateTime(topic.updatedAt) : '—'}</p>
              </InfoRow>
            </div>
          </CardPanel>
    ...
    <div className="rounded-xl border border-[#e8ecf0] bg-white shadow-sm self-start overflow-hidden">
      ...
      <div className="divide-y divide-[#f5f5f5]">
        <div className="px-5 py-3.5">
          <Link to={`/lecture/tracks/${trackId}`} className="text-[14px] font-bold text-[#064f5d] hover:underline">{track.title}</Link>
        </div>
...
function R({ icon, label, value, mono = false }) {
  return (
    <p className="flex items-center gap-2 text-[13px]">
      ...
      <span className={`ml-auto font-semibold ${mono ? 'text-[12px] font-medium' : ''}`}>{value}</span>
    </p>
  )
}
